    ...
    return BbPromise
      ...
      .then(stats => {
        ...
        if (!Array.isArray(stats.stats) && stats.compilation.errors.length) {
          ...
          stats.forEach((stat, index) => {
            if (Array.isArray(this.webpackConfig)) {
              console.log('static, ', this.webpackConfig[index].static);
              if (this.webpackConfig[index].service) {
                serviceIndex = index;
              } else if (this.webpackConfig[index].static) {
                staticIndex = index;
              }
            }
        ...
        if (serviceIndex || staticIndex) {
          ...
          if (staticIndex) {
            console.log('static index available at ', stats[staticIndex].compilation.compiler.outputPath);
